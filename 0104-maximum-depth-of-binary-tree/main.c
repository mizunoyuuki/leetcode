...
int DFSearch(struct TreeNode* node, int depth){
	...
	if (left_result == -1 && right_result == -1) return depth;
	else if (left_result != -1 && right_result == -1) return left_result;
	else if (left_result == -1 && right_result != -1) return right_result;
	else (left_result != -1 && right_result != -1){
